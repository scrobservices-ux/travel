...
let ui = { tab: "dashboard", sittingId: null, levyId: null, newType: "family", newStyle: "rotating", shareChoice: {}, payPeriod: "annual" };
...
const I18N = {
  en: {
    ...
    per_year: "/ year", per_month: "/ month", free: "Free",
    annual: "Annual", monthly: "Monthly", founding_house: "Founding House (−50%)",
    founding_50: "Founding House −50%",
    price_for_members: "{n} members × {per}/yr (floor & cap applied)",
    ...
    // savings & loans
    nav_savings: "Savings & Loans",
    caisse_value: "Caisse value", cash_available: "Cash available now",
    on_loan: "Out on loan", interest_earned: "Interest earned",
    total_savings: "Total savings",
    savings_intro: "{pct}% of every contribution is kept in the common savings. The caisse lends to members at {rate}%/month — the interest grows everyone's savings.",
    savings_off: "Common savings is off for this house. Turn it on in Settings (set a savings % above 0) to keep part of each contribution as a shared, lending caisse.",
    open_settings: "Open Settings",
    give_loan: "Give a loan from the caisse", borrower: "Borrower", loan_amount: "Loan amount",
    give_loan_btn: "Give loan", loans: "Loans", no_loans: "No loans yet.",
    principal: "Principal", outstanding: "Outstanding", monthly_interest_short: "Interest/mo",
    record_repayment: "Record repayment", interest_due_lbl: "Interest ({rate}%/mo)", principal_paid: "Principal repaid",
    settled: "Settled", loan_given_to: "Loan to",
    year_end: "Year-end share-out", shareout_intro: "Each member gets their own savings back plus a share of the interest, in proportion to what they saved.",
    deposits_col: "Savings", interest_share: "Interest share", total_share: "To receive",
    record_shareout: "Record share-out (pay everyone)", shareout_done: "Share-out recorded — caisse paid back to members.",
    exceeds_cash: "That is more than the cash in the caisse ({cash}). Give the loan anyway?",
    loans_note: "Note: {amt} is still out on loan and must be collected before everyone can be fully paid.",
    savings_pct_label: "Common savings — % of each contribution (0 = off)",
    loan_rate_label: "Loan interest — % per month",
    no_savings_yet: "No savings in the caisse yet.",
    carried_in: "Carried in", this_cycle: "This year", your_stake: "Stake",
    cash_out: "Cash out", roll_over: "Roll over & stay", staying: "Staying", taking: "Taking out",
    choose_each: "Each member can take their money or roll it over to next year. Tap a member to switch.",
    close_cycle: "Close year & apply choices", cycle_closed: "Year closed. Money rolled over stays in the caisse for next year.",
  ...
  fr: {
    ...
    per_year: "/ an", per_month: "/ mois", free: "Gratuit",
    annual: "Annuel", monthly: "Mensuel", founding_house: "Maison fondatrice (−50%)",
    founding_50: "Maison fondatrice −50%",
    price_for_members: "{n} membres × {per}/an (plancher & plafond appliqués)",
    ...
    nav_savings: "Épargne & Prêts",
    caisse_value: "Valeur de la caisse", cash_available: "Liquide disponible",
    on_loan: "Prêté", interest_earned: "Intérêts gagnés",
    total_savings: "Épargne totale",
    savings_intro: "{pct}% de chaque cotisation est gardé dans l'épargne commune. La caisse prête aux membres à {rate}%/mois — les intérêts font grossir l'épargne de tous.",
    savings_off: "L'épargne commune est désactivée pour cette maison. Activez-la dans les Réglages (mettez un % d'épargne au-dessus de 0) pour garder une part de chaque cotisation comme caisse de prêt partagée.",
    open_settings: "Ouvrir les Réglages",
    give_loan: "Accorder un prêt depuis la caisse", borrower: "Emprunteur", loan_amount: "Montant du prêt",
    give_loan_btn: "Accorder le prêt", loans: "Prêts", no_loans: "Aucun prêt.",
    principal: "Capital", outstanding: "Restant dû", monthly_interest_short: "Intérêt/mois",
    record_repayment: "Enregistrer un remboursement", interest_due_lbl: "Intérêt ({rate}%/mois)", principal_paid: "Capital remboursé",
    settled: "Soldé", loan_given_to: "Prêt à",
    year_end: "Partage de fin d'année", shareout_intro: "Chaque membre récupère sa propre épargne plus une part des intérêts, au prorata de ce qu'il a épargné.",
    deposits_col: "Épargne", interest_share: "Part d'intérêts", total_share: "À recevoir",
    record_shareout: "Enregistrer le partage (payer tout le monde)", shareout_done: "Partage enregistré — caisse reversée aux membres.",
    exceeds_cash: "C'est plus que le liquide dans la caisse ({cash}). Accorder le prêt quand même ?",
    loans_note: "Note : {amt} est encore prêté et doit être récupéré avant de payer tout le monde.",
    savings_pct_label: "Épargne commune — % de chaque cotisation (0 = aucune)",
    loan_rate_label: "Intérêt de prêt — % par mois",
    no_savings_yet: "Aucune épargne dans la caisse pour l'instant.",
    carried_in: "Reporté", this_cycle: "Cette année", your_stake: "Mise",
    cash_out: "Retirer", roll_over: "Reporter & rester", staying: "Reste", taking: "Retire",
    choose_each: "Chaque membre peut retirer son argent ou le reporter à l'année suivante. Touchez un membre pour basculer.",
    close_cycle: "Clôturer l'année & appliquer", cycle_closed: "Année clôturée. L'argent reporté reste dans la caisse pour l'an prochain.",
  ...
  pcm: {
    ...
    per_year: "/ year", per_month: "/ month", free: "Free",
    annual: "Yearly", monthly: "Monthly", founding_house: "Founding House (−50%)",
    founding_50: "Founding House −50%",
    price_for_members: "{n} pipo × {per}/year (floor & cap dey)",
    ...
    nav_savings: "Savings & Loan",
    caisse_value: "Caisse value", cash_available: "Cash weh dey now",
    on_loan: "Dem don borrow", interest_earned: "Interest weh e make",
    total_savings: "Total savings",
    savings_intro: "{pct}% of every contribution di stay for di common savings. Di caisse di loan member dem at {rate}%/month — di interest di grow everybody savings.",
    savings_off: "Common savings dey off for dis njangi. Put am on for Settings (set savings % pass 0) make part of every contribution stay as savings weh dem fit borrow.",
    open_settings: "Open Settings",
    give_loan: "Give loan from di caisse", borrower: "Weh borrow", loan_amount: "Loan amount",
    give_loan_btn: "Give loan", loans: "Loan dem", no_loans: "No loan yet.",
    principal: "Principal", outstanding: "Weh remain", monthly_interest_short: "Interest/month",
    record_repayment: "Record payback", interest_due_lbl: "Interest ({rate}%/month)", principal_paid: "Principal weh dem pay",
    settled: "Don finish", loan_given_to: "Loan for",
    year_end: "End-of-year share", shareout_intro: "Every member di collect e own savings plus some interest, according to how much e save.",
    deposits_col: "Savings", interest_share: "Interest share", total_share: "Go collect",
    record_shareout: "Record di share (pay everybody)", shareout_done: "Share don record — caisse don go back for members.",
    exceeds_cash: "Na pass di cash weh dey for caisse ({cash}). Still give di loan?",
    loans_note: "Note: {amt} still dey for loan, dem must collect am before everybody fit collect finish.",
    savings_pct_label: "Common savings — % of every contribution (0 = off)",
    loan_rate_label: "Loan interest — % per month",
    no_savings_yet: "No savings for di caisse yet.",
    carried_in: "Weh carry come", this_cycle: "Dis year", your_stake: "Stake",
    cash_out: "Take comot", roll_over: "Roll over & stay", staying: "E stay", taking: "E take comot",
    choose_each: "Every member fit take e money or roll am over for next year. Touch member make e change.",
    close_cycle: "Close di year & apply", cycle_closed: "Year don close. Money weh dem roll over di stay for caisse for next year.",
...
// Per-member annual pricing with a floor and a cap (see PRICING.md).
const PLANS = {
  start:    { key: "start",    perMember: 0,    floor: 0,     cap: 0,      maxMembers: 15,       premium: false },
  standard: { key: "standard", perMember: 1000, floor: 10000, cap: 50000,  maxMembers: Infinity, premium: true },
  elite:    { key: "elite",    perMember: 3500, floor: 50000, cap: 200000, maxMembers: Infinity, premium: true },
};

const FOUNDING_DISCOUNT = 0.5;     // first 200 paying groups — 50% off, kept for life
const MONTHLY_MARKUP = 1.25;       // monthly costs ~25% more than 1/12 of annual

const YEAR_MS = 365 * 24 * 60 * 60 * 1000;
const MONTH_MS = 31 * 24 * 60 * 60 * 1000;

// Annual price for a plan given the house's size, floor/cap applied.
function annualPrice(planKey, members) {
  const p = PLANS[planKey];
  if (!p || !p.premium) return 0;
  const n = Math.max(1, members || 1);
  return Math.min(p.cap, Math.max(p.floor, p.perMember * n));
}
// What this house pays now, for the chosen period, with founding discount.
function planPrice(g, planKey, period) {
  const base = annualPrice(planKey, activeMembers(g).length);
  const annual = g.founding ? Math.round(base * (1 - FOUNDING_DISCOUNT)) : base;
  if (period === "monthly") return Math.round(annual / 12 * MONTHLY_MARKUP);
  return annual;
}
...
function icon(name, size) {
  ...
  const paths = {
    ...
    bank: '<path d="M3 9.5 12 4l9 5.5"/><path d="M5 10v8M10 10v8M14 10v8M19 10v8"/><path d="M3 21h18"/>',
    handcoins: '<circle cx="16" cy="7" r="3"/><path d="M2 19c1.5-2 4-3 6.5-3 1.6 0 3 .4 4 1"/><path d="M9 13l3.5 2.5c.8.6 2 .5 2.7-.2l3-3"/>',
  };
...
// Optional common-savings caisse. savingsRate 0 = feature off (the default).
function savingsRate(g) { return g.savingsRate ? g.savingsRate : 0; }
function loanRate(g) { return g.loanRate != null ? g.loanRate : 0.02; }

// The rotating pot keeps only the non-savings share of each contribution.
function sittingTotal(g, sittingId) {
  const r = savingsRate(g);
  return g.transactions.filter(t => t.sittingId === sittingId).reduce((s, t) => {
    if (t.type === "contribution" || t.type === "reversal-contribution") return s + t.amount * (1 - r);
    if (t.type === "fine") return s + t.amount;
    return s;
  }, 0);
}
...
// Member savings come from the savings-share of their contributions.
function caisseDeposits(g) {
  const r = savingsRate(g);
  return g.transactions.filter(t => t.type === "contribution" || t.type === "reversal-contribution")
    .reduce((s, t) => s + t.amount * r, 0);
}
function memberDeposits(g, mId) {
  const r = savingsRate(g);
  return g.transactions.filter(t => (t.type === "contribution" || t.type === "reversal-contribution") && t.memberId === mId)
    .reduce((s, t) => s + t.amount * r, 0);
}
function interestEarned(g) {
  return g.transactions.filter(t => t.type === "loan-interest-paid").reduce((s, t) => s + t.amount, 0);
}
function savingsPaidOut(g) {
  return -g.transactions.filter(t => t.type === "savings-payout").reduce((s, t) => s + t.amount, 0);
}
function loanOutstanding(g, loanId) {
  const loan = (g.loans || []).find(l => l.id === loanId);
  if (!loan) return 0;
  const repaid = g.transactions.filter(t => t.loanId === loanId && t.type === "loan-principal-repaid").reduce((s, t) => s + t.amount, 0);
  return Math.max(0, loan.principal - repaid);
}
function totalOnLoan(g) { return (g.loans || []).reduce((s, l) => s + loanOutstanding(g, l.id), 0); }
// Cash physically in the caisse right now.
function caisseCash(g) {
  const flows = g.transactions
    .filter(t => ["loan-disbursed", "loan-principal-repaid", "loan-interest-paid", "savings-payout"].includes(t.type))
    .reduce((s, t) => s + t.amount, 0);
  return caisseDeposits(g) + flows;
}
// Total equity of the caisse: deposits + interest earned, minus what's been shared out.
function caisseValue(g) { return caisseDeposits(g) + interestEarned(g) - savingsPaidOut(g); }
function loanMemberId(g, loanId) { const l = (g.loans || []).find(x => x.id === loanId); return l ? l.memberId : null; }

// --- savings cycles (roll-over / cash-out) ---
// A "cycle" is usually one year. Deposits since cycleStart count toward this
// cycle's stake; carried[memberId] is what a member rolled over from before.
function cycleStart(g) { return g.cycleStartTs || 0; }
function thisCycleDeposits(g, mId) {
  const r = savingsRate(g);
  return g.transactions
    .filter(t => (t.type === "contribution" || t.type === "reversal-contribution") && t.memberId === mId && t.ts >= cycleStart(g))
    .reduce((s, t) => s + t.amount * r, 0);
}
function carriedOf(g, mId) { return (g.carried && g.carried[mId]) || 0; }
function memberStake(g, mId) { return carriedOf(g, mId) + thisCycleDeposits(g, mId); }
function totalStake(g) { return activeMembers(g).reduce((s, m) => s + memberStake(g, m.id), 0); }
function interestThisCycle(g) {
  return g.transactions.filter(t => t.type === "loan-interest-paid" && t.ts >= cycleStart(g)).reduce((s, t) => s + t.amount, 0);
}
// What a member is entitled to at year-end: their stake + pro-rata interest share.
function entitlement(g, mId) {
  const ts = totalStake(g), st = memberStake(g, mId);
  return st + (ts > 0 ? interestThisCycle(g) * st / ts : 0);
}

// --- tier gates ---
function canSavings(g) { return planStatus(g).effective.premium; }              // Standard or Elite
function isEliteActive(g) { const ps = planStatus(g); return ps.plan.key === "elite" && ps.status === "active"; }
...
function viewSavings(g) {
  ...
  if (savingsRate(g) <= 0) {
    return `<div class="card"><h3>${icon("bank", 17)} ${t("nav_savings")}</h3>
      ...
      <button class="btn" onclick="App.tab('settings')">${t("open_settings")}</button></div>`;
  }
  const r = Math.round(savingsRate(g) * 100);
  const rate = +(loanRate(g) * 100).toFixed(2);
  const D = caisseDeposits(g), I = interestEarned(g);

  const loans = (g.loans || []).slice().reverse();
  const loanRows = loans.map(l => {
    const out = loanOutstanding(g, l.id);
    const done = out <= 0;
    const suggest = Math.round(out * loanRate(g));
    return `
    <tr class="${done ? "paid" : ""}">
      <td>${esc(memberName(g, l.memberId))}<div class="muted" style="font-size:12px">${l.date}</div></td>
      <td class="num">${fmt(l.principal)}</td>
      <td class="num">${done ? `<span class="pill green">${t("settled")}</span>` : fmt(out)}</td>
      <td class="num">${done ? "—" : fmt(suggest)}</td>
      <td class="right">${done ? "" : `<button class="btn small gold" onclick="App.openRepay('${l.id}')">${t("record_repayment")}</button>`}</td>
    </tr>`;
  }).join("");

  // year-end share-out / roll-over preview (cycle-aware)
  const elite = isEliteActive(g);
  const tStake = totalStake(g), Ic = interestThisCycle(g);
  const shareRows = activeMembers(g).map(m => {
    const stake = memberStake(g, m.id);
    if (stake <= 0) return "";
    const ent = entitlement(g, m.id);
    const intShare = ent - stake;
    const choice = ui.shareChoice[m.id] === "out" ? "out" : "roll";
    const toggle = elite
      ? `<td class="right"><button class="btn small ${choice === "out" ? "warn" : "ghost"}" onclick="App.toggleShare('${m.id}')">${choice === "out" ? t("taking") : t("staying")}</button></td>`
      : "";
    return `<tr><td>${esc(m.name)}</td><td class="num">${fmt(carriedOf(g, m.id))}</td><td class="num">${fmt(thisCycleDeposits(g, m.id))}</td><td class="num">${fmt(intShare)}</td><td class="num"><b>${fmt(ent)}</b></td>${toggle}</tr>`;
  }).join("");

  return `
  <div class="statgrid">
    <div class="stat blue"><div class="s-icon">${icon("bank", 20)}</div><div><div class="label">${t("caisse_value")}</div><div class="value">${fmt(caisseValue(g))}</div></div></div>
    <div class="stat green"><div class="s-icon">${icon("coins", 20)}</div><div><div class="label">${t("cash_available")}</div><div class="value">${fmt(caisseCash(g))}</div></div></div>
    <div class="stat terra"><div class="s-icon">${icon("handcoins", 20)}</div><div><div class="label">${t("on_loan")}</div><div class="value">${fmt(totalOnLoan(g))}</div></div></div>
    <div class="stat gold"><div class="s-icon">${icon("spark", 20)}</div><div><div class="label">${t("interest_earned")}</div><div class="value">${fmt(I)}</div></div></div>
  </div>
  <p class="muted" style="margin:2px 4px 14px">${icon("bank", 14)} ${t("savings_intro", { pct: r, rate: rate })}</p>

  <div class="card">
    <h3>${icon("handcoins", 17)} ${t("give_loan")}</h3>
    <div class="formrow">
      <label class="field"><span>${t("borrower")}</span>
        <select id="loanMember">${activeMembers(g).map(m => `<option value="${m.id}">${esc(m.name)}</option>`).join("")}</select>
      </label>
      <label class="field"><span>${t("loan_amount")}</span><input type="number" id="loanAmount" placeholder="50000"></label>
    </div>
    <button class="btn" onclick="App.giveLoan()">${icon("handcoins", 15)} ${t("give_loan_btn")}</button>
  </div>

  <div class="card">
    <h3>${icon("coins", 17)} ${t("loans")}</h3>
    ${loans.length ? `<div class="scrollwrap"><table><thead><tr><th>${t("borrower")}</th><th class="num">${t("principal")}</th><th class="num">${t("outstanding")}</th><th class="num">${t("monthly_interest_short")}</th><th></th></tr></thead><tbody>${loanRows}</tbody></table></div>`
      : `<p class="muted">${t("no_loans")}</p>`}
  </div>

  <div class="card">
    <h3>${icon("users", 17)} ${t("year_end")}</h3>
    <p class="muted">${elite ? t("choose_each") : t("shareout_intro")}</p>
    ${!elite ? `<p class="muted" style="font-size:12.5px">${icon("crown", 13)} ${t("rollover_premium_note")}</p>` : ""}
    ${totalOnLoan(g) > 0 ? `<p class="pill gold" style="display:inline-block">${t("loans_note", { amt: fmt(totalOnLoan(g)) })}</p>` : ""}
    ${shareRows ? `<div class="scrollwrap mt"><table><thead><tr><th>${t("member")}</th><th class="num">${t("carried_in")}</th><th class="num">${t("this_cycle")}</th><th class="num">${t("interest_share")}</th><th class="num">${t("total_share")}</th>${elite ? "<th></th>" : ""}</tr></thead><tbody>${shareRows}</tbody></table></div>
      ${elite
        ? `<button class="btn gold mt" onclick="App.closeCycle(false)">${icon("download", 15)} ${t("close_cycle")}</button>`
        : `<button class="btn gold mt" onclick="App.closeCycle(true)">${icon("download", 15)} ${t("record_shareout")}</button>`}`
      : `<p class="muted">${t("no_savings_yet")}</p>`}
  </div>`;
}
...
function planCard(g, key) {
  ...
  const period = ui.payPeriod || "annual";
  const n = activeMembers(g).length;
  let priceHtml;
  if (key === "start") {
    priceHtml = `<div class="plan-price">${t("free")}</div>`;
  } else {
    const amount = planPrice(g, key, period);
    const full = period === "monthly" ? planPrice(g, key, "monthly") : annualPrice(key, n);
    const discounted = g.founding;
    priceHtml = `<div class="plan-price">${fmt(amount)} <span style="font-size:14px;color:var(--muted);font-weight:600">${period === "monthly" ? t("per_month") : t("per_year")}</span></div>
      <div class="muted" style="font-size:11.5px;margin:-8px 0 12px">${t("price_for_members", { n: n, per: fmt(p.perMember) })}${discounted ? ` · <b style="color:var(--terra)">${t("founding_50")}</b>` : ""}</div>`;
  }
...
function viewPlan(g) {
  ...
  <p class="muted" style="margin:0 0 12px;padding:0 4px">${icon("spark", 14)} ${t("plan_pay_at_shareout")}</p>
  <div class="period-toggle">
    <button class="${(ui.payPeriod || "annual") === "annual" ? "on" : ""}" onclick="App.setPeriod('annual')">${t("annual")}</button>
    <button class="${ui.payPeriod === "monthly" ? "on" : ""}" onclick="App.setPeriod('monthly')">${t("monthly")}</button>
    <label class="founding-toggle"><input type="checkbox" ${g.founding ? "checked" : ""} onclick="App.toggleFounding()"> ${t("founding_house")}</label>
  </div>
  <div class="plan-grid">
    ${planCard(g, "start")}
    ${planCard(g, "standard")}
...
const App = {
  ...
  goHome() { render(); },
  selectGroup(id) { state.activeGroupId = id; ui = { tab: "dashboard", sittingId: null, levyId: null, newType: ui.newType, shareChoice: {} }; save(); render(); },
  ...
  showNewGroup(presetType) {
    ...
    openModal(`
      ...
      <label class="field"><span>${t("savings_pct_label")}</span><input type="number" id="ngSavings" value="0" min="0" max="90"></label>
  ...
  saveSettings() {
    ...
    const sv = document.getElementById("gSavings");
    const wantSavings = sv ? Math.max(0, Math.min(90, Number(sv.value) || 0)) / 100 : savingsRate(g);
    if (wantSavings > 0 && !canSavings(g)) {
      // Savings caisse is a paid (Standard/Elite) feature — basic njangi stays free.
      g.savingsRate = 0;
      const lr0 = document.getElementById("gLoanRate");
      if (lr0) g.loanRate = Math.max(0, Number(lr0.value) || 0) / 100;
      if (!g.loans) g.loans = [];
      save(); render();
      return this.showPaywall("savings");
    }
  ...
  giveLoan() {
    ...
    if (!amount) return alert(t("enter_amount"));
    const cash = caisseCash(g);
    if (amount > cash && !confirm(t("exceeds_cash", { cash: fmt(cash) }))) return;
    g.loans = g.loans || [];
    const loan = { id: uid(), memberId, principal: amount, date: todayISO() };
    g.loans.push(loan);
    addTx(g, { type: "loan-disbursed", amount: -amount, loanId: loan.id, memberId, method: "MoMo/OM or cash", note: "Loan given" });
    render();
  },

  openRepay(loanId) {
    const g = activeGroup();
    const out = loanOutstanding(g, loanId);
    const suggest = Math.round(out * loanRate(g));
    const rate = +(loanRate(g) * 100).toFixed(2);
    openModal(`
      <h3>${icon("handcoins", 20)} ${t("record_repayment")}</h3>
      <p class="muted" style="margin-top:-6px">${t("loan_given_to")} <b>${esc(memberName(g, loanMemberId(g, loanId)))}</b> · ${t("outstanding")}: <b>${fmt(out)}</b></p>
      <label class="field"><span>${t("interest_due_lbl", { rate: rate })}</span><input type="number" id="repayInterest" value="${suggest}"></label>
      <label class="field"><span>${t("principal_paid")}</span><input type="number" id="repayPrincipal" value="0" placeholder="0"></label>
      <div class="row-actions" style="justify-content:flex-start">
        <button class="btn gold" onclick="App.recordRepay('${loanId}')">${icon("check", 15)} ${t("record_repayment")}</button>
        <button class="btn ghost" onclick="App.closeModal()">${t("close")}</button>
      </div>`);
  },
  ...
  recordRepay(loanId) {
    const g = activeGroup();
    const interest = Number(document.getElementById("repayInterest").value) || 0;
    let principal = Number(document.getElementById("repayPrincipal").value) || 0;
    const out = loanOutstanding(g, loanId);
    if (principal > out) principal = out;
    if (interest <= 0 && principal <= 0) return alert(t("enter_amount"));
    const mId = loanMemberId(g, loanId);
    if (interest > 0) addTx(g, { type: "loan-interest-paid", amount: interest, loanId, memberId: mId, method: "cash", note: "Loan interest" });
    if (principal > 0) addTx(g, { type: "loan-principal-repaid", amount: principal, loanId, memberId: mId, method: "cash", note: "Loan principal" });
    this.closeModal(); render();
  },

  toggleShare(mId) {
    ui.shareChoice[mId] = (ui.shareChoice[mId] === "out") ? "roll" : "out";
    render();
  },

  // Close the year. forceOut=true → everyone cashes out (Standard).
  // forceOut=false → per-member choice from ui.shareChoice (Elite roll-over).
  closeCycle(forceOut) {
    const g = activeGroup();
    if (totalStake(g) <= 0) return alert(t("no_savings_yet"));
    const choiceOf = m => forceOut ? "out" : (ui.shareChoice[m.id] === "out" ? "out" : "roll");
    const cashNeeded = activeMembers(g).reduce((s, m) => s + (choiceOf(m) === "out" ? entitlement(g, m.id) : 0), 0);
    if (cashNeeded > caisseCash(g) + 1 && !confirm(t("exceeds_cash", { cash: fmt(caisseCash(g)) }))) return;
    if (!confirm((forceOut ? t("record_shareout") : t("close_cycle")) + " ?")) return;

    const interestRec = interestThisCycle(g);
    const newCarried = Object.assign({}, g.carried || {});
    for (const m of activeMembers(g)) {
      const ent = entitlement(g, m.id);
      if (choiceOf(m) === "out") {
        if (ent > 0) addTx(g, { type: "savings-payout", amount: -ent, memberId: m.id, method: "MoMo/OM or cash", note: forceOut ? "Year-end share-out" : "Year-end cash-out" });
        newCarried[m.id] = 0;
      } else {
        newCarried[m.id] = ent; // rolls over, no cash moves
      }
    }
    g.carried = newCarried;
    g.cycleStartTs = Date.now();
    g.cycles = g.cycles || [];
    g.cycles.push({ date: todayISO(), interest: interestRec });
    ui.shareChoice = {};
    save(); render();
    alert(forceOut ? t("shareout_done") : t("cycle_closed"));
  },
  ...
  setPeriod(p) { ui.payPeriod = p; render(); },
  toggleFounding() { const g = activeGroup(); g.founding = !g.founding; save(); render(); },
  ...
  startPay(key) {
    ...
    openModal(`
      ...
      <div class="pay-amount">${fmt(amount)} <span>${period === "monthly" ? t("per_month") : t("per_year")}</span></div>
      <div class="muted" style="margin:-12px 0 14px">${t("price_for_members", { n: n, per: fmt(PLANS[key].perMember) })}${g.founding ? ` · <b style="color:var(--terra)">${t("founding_50")}</b>` : ""}</div>
  ...
  confirmPay(key) {
    ...
    const period = ui.payPeriod || "annual";
    const amount = planPrice(g, key, period);
    const ref = (document.getElementById("payRef") ? document.getElementById("payRef").value || "" : "").trim();
    g.plan = key;
    g.validUntil = new Date(Date.now() + (period === "monthly" ? MONTH_MS : YEAR_MS)).toISOString();
    g.subHistory = g.subHistory || [];
    g.subHistory.push({ date: todayISO(), plan: key, period, amount, members: activeMembers(g).length, ref, method: "MoMo/OM" });
    save(); this.closeModal(); render();
