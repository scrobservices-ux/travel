...
function viewSavings(g) {
  ...
  // year-end share-out preview
  const shareRows = activeMembers(g).map(m => {
    const dep = memberDeposits(g, m.id);
    if (dep <= 0) return "";
    const share = D > 0 ? I * dep / D : 0;
    return `<tr><td>${esc(m.name)}</td><td class="num">${fmt(dep)}</td><td class="num">${fmt(share)}</td><td class="num"><b>${fmt(dep + share)}</b></td></tr>`;
  }).join("");
  ...
  <div class="card">
    ...
    <p class="muted">${t("shareout_intro")}</p>
    ${totalOnLoan(g) > 0 ? `<p class="pill gold" style="display:inline-block">${t("loans_note", { amt: fmt(totalOnLoan(g)) })}</p>` : ""}
    ${shareRows ? `<div class="scrollwrap mt"><table><thead><tr><th>${t("member")}</th><th class="num">${t("deposits_col")}</th><th class="num">${t("interest_share")}</th><th class="num">${t("total_share")}</th></tr></thead><tbody>${shareRows}</tbody></table></div>
      <button class="btn gold mt" onclick="App.recordShareout()">${icon("download", 15)} ${t("record_shareout")}</button>`
...
const App = {
  ...
  saveSettings() {
    ...
    if (sv) g.savingsRate = Math.max(0, Math.min(90, Number(sv.value) || 0)) / 100;
  ...
  recordShareout() {
    const g = activeGroup();
    const D = caisseDeposits(g), I = interestEarned(g);
    if (D <= 0) return alert(t("no_savings_yet"));
    if (!confirm(t("record_shareout") + " ?")) return;
    for (const m of activeMembers(g)) {
      const dep = memberDeposits(g, m.id);
      if (dep <= 0) continue;
      const total = dep + I * dep / D;
      addTx(g, { type: "savings-payout", amount: -total, memberId: m.id, method: "MoMo/OM or cash", note: "Year-end savings share-out" });
    }
    render(); alert(t("shareout_done"));
  },
