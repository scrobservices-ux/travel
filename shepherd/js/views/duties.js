...
(function (global) {
  ...
  function activeTypes() {
    ...
    state.types = S.DUTY_TYPES.map(function (d) { return d.id; })
      .filter(function (id) { return id !== 'zoom'; });
    return state.types;
  }
  ...
  function setDuty(date, meeting, typeId, personId, groupId) {
    ...
    Store.update({
      action: 'duty.set',
      summary: S.dutyType(typeId).name + ' — ' + (personId ? Store.name(personId) : groupId ? (Store.group(groupId) || {}).name : 'cleared') + ' ' + U.fmtDate(date)
    }, function (st) {
      if (!personId && !groupId) {
  ...
  Views.duties = {
    ...
    render: function (root) {
      ...
      S.DUTY_TYPES.forEach(function (dt) {
        var on = activeTypes().indexOf(dt.id) !== -1;
        chips.appendChild(UI.btn(dt.name, {
          sm: true, pressed: on,
          onClick: function () {
            state.types = on ? activeTypes().filter(function (x) { return x !== dt.id; })
      ...
      dates.forEach(function (r) {
        activeTypes().forEach(function (t) { if (!dutyFor(r.date, t)) gaps.push(r.date + ' · ' + S.dutyType(t).name); });
      });
  ...
  function exportCsv() {
    var dates = meetingDates(state.weeks);
    var rows = [['Date', 'Meeting'].concat(activeTypes().map(function (t) { return S.dutyType(t).name; }))];
    dates.forEach(function (r) {
